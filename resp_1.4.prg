...
cMolde1 := ( "c:\resp\run\molde_sragx.dbf" )
...
replace sem_epid with "sem 0" + alltrim(str(f))
...
replace sem_epid with "sem " + alltrim(str(g))
replace srag_nespe with var_sra_ne
replace srag_covid with var_sra_cov
replace srag_influ with var_sra_inf
replace srag_ou_ag with var_sra_o_a
...
@ 13,0 say "Criando arquivo para salvar as informacoes de Influenza..."

cMolde2 := ( "c:\resp\run\molde_influ.dbf" )
...
aStruct := { { "sem_epid","C", 6, 0 }, ;
			 ...
			 { "b_yamagath", "N", 10, 0 }, ;
			 { "b_outro", "N", 10, 0 }}
			 dbcreate (( cMolde2 ),aStruct )

@ 14,0 say "Gerando dados das semanas epidemiologicas do ano " + alltrim( cAno ) + "."
...
@ 15,0 say "Semana:" + "0" + alltrim(str(h)) + "..."
...
replace sem_epid with "sem 0" + alltrim(str(h))
...
replace sem_epid with "sem " + alltrim(str(i))
replace a_h1n1 with var_a_h1n1
replace a_h3n2 with var_a_h3n2
replace a_tipado with var_a_pado
replace a_tipavel with var_a_pavel
